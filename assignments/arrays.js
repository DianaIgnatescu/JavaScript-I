...
function findCar(inventory, key, value) {
  for (let i=0; i < inventory.length; i++) {
    if (inventory[i][key] === value) {
      return inventory[i];
    }
  }
  // return null;
...
findCar(inventory, 'id', 33);
// console.log(findCar(inventory, 'id', 33));
...
carModels = inventory.map(order => order.car_model.toLowerCase())
...
oldCars = inventory.filter(year => year.car_year < 2000)
console.log(oldCars);
